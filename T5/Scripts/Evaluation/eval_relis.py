...
direct = "/Users/shashankgupta/Documents/Raredis/T5/no_copy/rel_is/predictions/t5_3b"
df = pd.read_csv(os.path.join(direct,"predictions.csv"))
...
def convert_relis_sentence(sentence):
    ...
    if segs is not None:
        ...
        if relation == "hyponym":
            ...
            rel = "produces"
        elif relation in ["synonym", "synonyms"]:
            rel = "is_synon"
...
def do_eval(preds, golden):
    ...
    # df.to_csv(os.path.join(direct,"error_analysis_rel_type_latest.csv"),index=False)


gold_lines = []
all_lines = []
for x in range(len(df)):
    gold = df["gold"][x].strip()
