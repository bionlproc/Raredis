...
local model_name = "microsoft/BiomedNLP-PubMedBERT-large-uncased-abstract";

local max_length = 512;       // Max length of input text
local max_steps = 460;         // Max number of decoding steps

local num_epochs = 150;        // Number of training epochs
local batch_size = 2;         // Per-GPU batch size
local grad_acc_steps = 2;     // Number of training steps before backpropagating gradients
...
local length_penalty = 1.5;   // >1.0 favours longer decodings and <1.0 shorter (test time only)
...
local validation_start = std.max(std.floor(num_epochs) - 50, 0);
