...
class _PredictManager:
    ...
    def _maybe_print_to_console_and_file(self,
        ...
        if self._output_file is not None:
            # self._output_file.write(model_input +  "\n")
            # self._output_file.write(prediction)
            temp = model_input[:-1]
            temp = temp + ", \"prediction\": " + prediction.strip() + "}\n"
            self._output_file.write(temp)

            # temp = model_input[:-1]
            # temp = temp + ", \"prediction\": " + prediction.strip() + "}"
            # with open(self._output_file.name, 'w') as json_file:
            #     json_file.write(temp + '\n')

            # temp = model_input[:-1]
            # temp = temp + ", \"prediction\": " + prediction.strip() + "}"
            # # Check if the file exists
            # if not os.path.exists(self._output_file.name):
            #     with open(self._output_file.name, 'w') as json_file:
            #         pass  # Create an empty file
            #
            # # Remove empty lines from the beginning of the file
            # with open(self._output_file.name, 'r') as json_file:
            #     lines = json_file.readlines()
            #
            # with open(self._output_file.name, 'w') as json_file:
            #     json_file.writelines(line for line in lines if line.strip())
            #
            # # Append JSON strings to the file in a loop
            # with open(self._output_file.name, 'a') as json_file:
            #         json_file.write(temp + '\n')

            # Convert the string to JSON format
            # json_data = json.loads(model_input)
