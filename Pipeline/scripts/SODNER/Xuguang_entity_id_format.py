...
daiACL2020_preprocess_dir = '/Users/shashankgupta/Documents/Raredis/dai_et_al/corrected_anno_latest/final_correction_data/truncated_text_format/xuguang_format/data/'

# Temporary directory
temp_output_dir = '/Users/shashankgupta/Documents/Raredis/SODNER/corrected_ano_preprocessfile/Xuguang_format/truncated_doc'

# Output directory
output_dir = '/Users/shashankgupta/Documents/Raredis/SODNER/corrected_ano_preprocessfile/Xuguang_format/truncated_doc'

stanfordcorenlp_dir = '/Users/shashankgupta/Documents/Raredis/SODNER/stanford-corenlp-4.5.0/'
...
def do_stat(file_path, dataset_type, output=False, output_path=''):
    ...
    with open(file_path) as fp:
        for line in fp:
            line = line.strip()
            if step == 0:
                sentence["doc"] = line + "_" + dataset_type + str(ct_sentence)
            elif step == 1:
                # words
                ...
                if len(line) != 0:
                    ...
                    for dai_en in dai_entities:
                        ...
                        entity = {}
                        temp_list = []
                        id = offset_type[0][:offset_type[0].find(r',')]
                        offsets = offset_type[1].split(',')
                        assert len(offsets) % 2 == 0
                        for i in range(0, len(offsets), 2):
                            xyz = []
                            xyz.append(int(offsets[i]))
                            xyz.append(int(offsets[i + 1]))
                        ...
                        temp_list.append(offset_type[2])
                        entity[id] = temp_list
                ...
                # relations
                if len(line) != 0:
                    dai_relations = line.split('|')
                    temp_list_rel = []
                    for dai_rel in dai_relations:
                        rel = dai_rel.split(' ')
                        rels = {}
                        rels[rel[2]] = [rel[0], rel[1]]
                        temp_list_rel.append(rels)

                    sentence["relations"] = temp_list_rel
...
def transfer_into_dygie(instances, output_file):
    ...
    for idx, instance in enumerate(tqdm(instances)):
        ...
        try:
            nlp_res_raw = nlp.annotate(' '.join(instance['tokens']),
