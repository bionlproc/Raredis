...
def find_token_starting_at_offset(doc, offset, token_boundaries):
    ...
    if (doc, offset - adjust) in token_boundaries:
        print(
            "Cannot find original offset (%d) in document (%s), so use (%d) instead." % (offset, doc, offset - adjust))
...
if __name__ == "__main__":
    ...
    token_start, token_end = load_token_boundaries(args.output_filepath_token)

    with open(args.output_tokens_ann, "w") as out_f:
        with open(args.output_filepath) as in_f:
            for line in in_f:
                sp = line.strip().split("\t")
                ...
                # Added ent_id
                doc, label, indices, mention, ent_id = sp
                ...
                out_f.write("%s\t%s\t%s\t%s\t%s\t%s\n" % (doc, sent_idx, label, token_indices, mention,ent_id))
